...
3. **Every chain step MUST include `"reads"` override** — Worker steps use `"reads": ["taskN.md"]` which both prevents defaultReads (context.md, plan.md) pre-loading AND auto-injects `[Read from: taskN.md]` so the Worker reads its task file automatically. Planner step uses `"reads": false` to prevent all pre-loading (Planner reads T₀ from the task prompt and may read Target Files using `read` tool).
...
8. **Worker reads only its own task file** — `reads: ["taskN.md"]` in the chain step auto-injects the task file. Worker does not need to see other Workers' tasks.
...
**No-op Worker**: if Worker K's task file is not found, Worker outputs "No task assigned. No files modified." and stops immediately. Do NOT retry the read.
