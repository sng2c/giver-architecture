...
3. **Every chain step MUST include `"reads"` override** — Worker steps use `"reads": ["taskN.md"]` which prevents defaultReads (context.md, plan.md) pre-loading and auto-injects `[Read from: taskN.md]` so the Worker receives its task file. Planner step uses `"reads": false` to prevent all pre-loading.
...
6. **Planner is in the chain** — P writes task1.md through taskN.md in the chain directory (same directory as this task file). N depends on logical modification groups, not file count. Chain always has 10 Worker slots. Unused Workers (no taskK.md) return no-op RESULT immediately.
