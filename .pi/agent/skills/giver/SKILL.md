...
name: giver
version: "3.6.3"
description: 'The Giver v3.6.3. Target Verification scope. reads:["taskN.md"] auto-inject. P-Wx10 chain. Same file OK across Workers. No Scout in chain. All subagents run fresh.'
...
Task #k (T_k) = Goal + Background + Past failures + Constraints + Target Files + Target Verification + Signatures  (Curated by Planner per Worker — saved to task{k}.md)
...
Target Verification (T₀) = All verification commands for the entire task (test files, type checks, build commands). Written by Giver. Examples: `npx vitest run src/__tests__/config.test.ts`, `pytest test_config.py`, `npx tsc --noEmit`, `cargo check`, `go build ./...`
Target Verification (Tₖ) = Verification commands this Worker should run to verify ONLY its own work. Subset assigned by Planner from T₀. Workers run ONLY their assigned verification — never the full suite.
...
Worker 1 → reads ["task1.md"] auto-injected, {previous} = Planner output
...
### Target Verification
[Verification commands for this entire task — test files, type checks, build commands. Per-language examples:]
[  TypeScript/JS: npx vitest run src/__tests__/config.test.ts, npx tsc --noEmit]
[  Python:             pytest test_config.py, mypy src/]
[  Rust:               cargo test --lib config, cargo check]
[  Go:                 go test ./pkg/..., go build ./...]
[  No tests:            npx tsc --noEmit, cargo check, go vet]
[]
[Planner assigns per-Worker subsets. Last Worker or Giver runs the full suite.]
