...
8. **Worker receives only its own task file** — `reads: ["taskN.md"]` in the chain step auto-injects the task file. If the file does not exist, Worker outputs no-op immediately (do NOT retry reading the task file). Worker does not see other Workers' tasks.
