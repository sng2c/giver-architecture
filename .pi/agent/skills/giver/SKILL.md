...
name: giver
version: "3.6.4"
description: 'The Giver v3.6.4. Minimum responsibility verification. reads:["taskN.md"] auto-inject. P-Wx10 chain. Same file OK across Workers. No Scout in chain. All subagents run fresh.'
...
9. **Each Worker verifies only its own changes** — this is a collaborative pipeline where each Worker handles its own scope. You know which files you created or modified, so verify those files work correctly. Other Workers verify their own scopes — running their tests would be redundant. This is rational, not arbitrary: you verify your work, they verify theirs. Use targeted test commands or type checks. Giver runs the full suite in Phase 5.
