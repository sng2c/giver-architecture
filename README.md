...
에이전트는 매 턴마다 이전의 모든 턴을 다시 읽는다. 이것이 복리의 시작이다.

1. **누적 재지불.** 도구 호출, 파일 읽기, 대화 내용이 컨텍스트에 쌓인다. 200번째 턴은 앞선 199개 턴을 모두 다시 지불한다. 단순 버그 수정 하나에 191K 토큰의 역사를 헤치고 들어가야 한다.

2. **노이즈가 신호를 삼킨다.** 컨텍스트가 길어질수록 이전 대화의 잡음이 현재 작업의 신호를 압도한다. 300번째 턴이면 에이전트는 옛날에 포기한 접근법, 이미 수정한 버그, 관련 없는 파일 읽기 기록까지 다시 처리해야 한다.

3. **Fork는 더하다.** fork 모드(상위 컨텍스트 상속)에서 자식은 부모의 누적된 전체 컨텍스트를 복사한 채 시작하고, 거기에 자신의 것까지 더한다. 최대 7.6M 토큰. 본질적으로 "더 큰 문제의 복사본"에서 시작하는 것이다.
