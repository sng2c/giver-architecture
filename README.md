...
### v1 베이스라인 (52세션, 65 서브에이전트 실행)

초기 프로토콜 적용 후 실제 세션 데이터로 베이스라인 측정. 구조적 이점은 확인되었으나, 구현 갭이 크게 드러남.

| 지표 | 측정값 | 비고 |
|------|--------|------|
| Monolithic 대비 토큰 절감 | **2.3×** | 620M vs 1,454M — 구조적 이점 실재 |
| Worker 이상적 범위 초과 | **97%** | 목표 ≤80K, 실제 중앙값 1.1M |
| `context:"fresh"` 명시 | **3%** | 132건 중 4건만 명시 |
| `context:"fork"` 사용 | **8건** | 최대 7.6M 토큰 누수 |
| context 미지정 | **90%** | 119건이 empty/default |
| Scout 평균 입력 | **275K** | 목표 ≤100K의 2.75× |
| Worker 평균 입력 | **1.9M** | 목표 ≤80K의 24× |
| giving of pain | **22건** | 활성, 에러 분류 16건 |
| 프로토콜 준수 (Phase 1.5) | **4%** | 2/52세션만 브랜치 생성 |

**핵심 진단:** 아키텍처의 구조적 이점(2.3× 절감)은 확인되었으나, 구현 갭이 절감 효과를 잠식. fork 누수(최대 7.6M), `context` 미지정(90%), 과도한 코드 리딩이 주원인.

> 상세 리포트: [`reports/baseline-v1-report.md`](reports/baseline-v1-report.md)

### v2 검증 (5체인, 12 서브에이전트 호출)

v1 베이스라인의 6개 개선항목 적용 후 초기 검증. 핵심 규칙(fork 금지, `context:"fresh"` 명시)의 효과를 실증.

| 지표 | v1 | v2 | 변화 |
|------|-----|-----|------|
| Fork 호출 | 8건 (6%) | **0건** | ✅ ELIMINATED |
| `context:"fresh"` 명시 | 3% | **100%** | ✅ 완전 |
| context 미지정 | 119건 (90%) | **0건** | ✅ ELIMINATED |
| Scout 평균 입력 | 275K | **105K** | ✅ **-62%** |
| Planner 평균 입력 | 691K | **513K** | ✅ -26% |
| Worker 평균 입력 | 1.9M | **1.4M** | ✅ -25% |
| Worker 버그 수정 | — | **8K 🟢** | ✅ 이상적 범위 도달 |
| Worker 이상적(≤80K) 비율 | 19% | **20%** | +1pp |

**핵심 성과:** fork 누수 완전 제거, `context:"fresh"` 100% 명시, Scout 62% 절감. 단일 파일 버그 수정(Chain 2)이 8K로 이상적 범위 도달 — v2가 설계 의도대로 작동함을 실증.

**잔존 과제:** Worker 3/5건이 여전히 🔴 과다. 전부 PTTPlugin.kt(1539줄 God Class) 추출 작업으로, 태스크 복잡도가 원인이지 아키텍처 결함이 아님. 함수 단위 분할(Worker당 3-5개 함수)로 각 Worker 200K 이하 수렴 예상.
