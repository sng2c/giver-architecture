...
│  역할: 대화 기록 보유, 브리핑 작성, 실패 전달, 결정 내림     │
│  토큰: 선형 증가 → 주기적 압축 → 수렴 (톱니 패턴)           │
│  입력: 사용자 메시지 + 하위 에이전트 결과                      │
├─────────────────────────────────────────────────────────────┤
│  Planner (Fresh)    │  Scout (Fresh)                         │
│  역할: 구현 계획     │  역할: 코드 정찰                         │
│  토큰: ≤500K        │  토큰: ≤100K (타겟팅 시)                │
│  입력: 브리프+리컨   │  입력: 타겟팅 지시만                    │
├─────────────────────────────────────────────────────────────┤
│  Worker (Fresh)                                              │
│  역할: 코드 변경 실행                                         │
│  토큰: ≤80K (이상적) — 브리프+리컨+대상 코드만               │
│  입력: plan.md + {previous} + 대상 파일                       │
└─────────────────────────────────────────────────────────────┘
```
...
| 모델 | 컨텍스트 성장 | 200K cap 시 총 입력 | Worker 입력 | 실패 전달 |
|------|-------------|-------------------|------------|----------|
| Monolithic | 기하급수 (재지불) | ~1,454M | 191K 누적 노이즈 | 없음 (같은 실수 반복) |
| Fork | 기하급수 (상속+증가) | 더 높음 | 최대 7.6M 상속 | 없음 |
| The Giver (이론) | 수렴 (톱니) | **~130M** | **≤80K** 브리프만 | giving of pain |
| The Giver (v1 실측) | 수렴 | **620M** | 중앙값 1.1M | 22건 |
