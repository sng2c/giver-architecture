...
| 층 | 에이전트 | 역할 | 토큰 예산 | 입력 | 컨텍스트 |
|----|---------|------|----------|------|----------|
| **상위** | Giver | 대화 기록 보유, 브리핑 작성, 실패 전달, 결정 | 선형 증가 → 주기적 압축 → 수렴 (톱니) | 사용자 메시지 + 하위 결과 | 전체 대화 보유 |
| **중간** | Planner (Fresh) | 구현 계획, Worker Briefing 작성 | ≤500K | Giver의 6섹션 브리프 + Scout 리컨 | 대화 기록 0 |
| **중간** | Scout (Fresh) | 코드 정찰, 타겟팅된 리컨 | ≤100K (타겟팅 시) | Giver의 타겟팅 지시만 | 대화 기록 0 |
| **하위** | Worker (Fresh) | 코드 변경 실행 | ≤80K (이상적) | plan.md + {previous} + 대상 파일 | 대화 기록 0 |
