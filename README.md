...
──────────────────────────────────────────────────────────────────────
모놀리식        ~200K/턴     누적 증가      0%       Giver 없이 단일 에이전트
...
**모놀리식 → v3.6.2: 단일 에이전트 누적 컨텍스트 → Worker 900토큰 (격리 파이프라인 효과)**

### 동일 과제 비교 (Redbis 44테스트, 실측)

| 지표 | 모놀리식 | v3.6.1 | **v3.6.2** |
|------|---------|--------|------------|
| W_bytes 평균 | 5,423B | 3,559B | **4,228B** |
| W_tokens 합 | 208K | 344K | 1,141K |
| W_tokens 최대 | 208K | 188K | 397K |
| tk/byte | 38× | 32× | **54×** |
| 총 tokens | 208K | 378K | 1,266K |
| 컨텍스트 누적 | 누적 ❌ | fresh ✅ | **fresh ✅** |
| 부분 재시도 | 불가 ❌ | Worker 단위 ✅ | **Worker 단위 ✅** |

> 모놀리식은 총 토큰이 적지만 컨텍스트 누적, 부분 재시도 불가. 과제가 커지면 품질 저하.
