...
| 주요 지표 | v2 결과 | v2.1 결과 | v2.2 결과 | 변화 및 평가 |
|-----------|---------|----------|----------|-------------|
| **Fork 호출** | 0건 | **0건** | **0건** | ✅ 격리 규칙 유지 |
| **`context:"fresh"` 준수율** | 100% | **100%** | **100%** | ✅ 격리 규칙 유지 |
| **Planner 평균** | 513K | **182K** | **61K** 🟢 | ✅ **88% 감소** (v2 대비) |
| **Planner 이상적(≤80K)** | 0% | **25%** | **100%** (1/1) | ✅ **Planner 100% 이상적** |
| Scout 평균 | 105K | **133K** | **59K** 🟢 | ✅ **44% 감소** (구조화 효과) |
| Worker 평균 | 1.4M | **975K** | **245K** | ✅ 75% 감소 (단일 체인) |
| **Worker 버그 수정** | 8K 🟢 | **38K** 🟢 | — | v2.2는 버그 수정 아님 |
| **Phase 0.5 준수율** | N/A | **75%** (3/4) | **관찰 필요** | v2.2는 Phase 0.5 미사용 |
| **모놀리식 대비 절감** | +13% 증가 | **-25% 절감** | **-78% 절감** | ✅ **78% 비용 절감 달성** |
...
|------|-----|-----|------|------|---------|
| Fork 호출 | 8건 | 0건 | **0건** | **0건** | ✅ 제거 유지 |
| `context:"fresh"` | 3% | 100% | **100%** | **100%** | ✅ 완전 유지 |
| Scout 평균 | 275K | 105K | **133K** | **59K** 🟢 | ✅ 78% 절감 |
| Planner 평균 | 691K | 513K | **182K** | **61K** 🟢 | ✅ **91% 절감** |
| Worker 평균 | 1.9M | 1.4M | **975K** | **245K** | ✅ 87% 절감 |
| Planner 이상적 | 0% | 0% | **25%** | **100%** | ✅ 완전 달성 |
| Worker 버그 수정 | — | 8K 🟢 | **38K** 🟢 | — | — |
| 모놀리식 대비 | +51% | +13% | **-25%** | **-78%** | — |

### 모놀리식 대비 버전별 절감 (단일 버그 수정 체인 기준)
...
**반복 실패 포함 시 절감 효과 증폭:** 동일 버그 4회 시도(Desync Bug) 기준 Giver 1,848K vs 모놀리식(누적) ~13,000K → **86% 절감**. 실패 시 컨텍스트 격리(`context:"fresh"`)가 핵심 이점.
