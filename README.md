...
| 방식 | Scout | Planner | Worker | 합계 | 모놀리식 대비 |
|------|------:|--------:|-------:|-----:|:------------:|
| 모놀리식(추정) | — | — | 1,906K | 1,906K | 기준(0%) |
| Giver v1 | 275K | 691K | 1,906K | 2,872K | +51% 증가 |
| Giver v2 | 105K | 513K | 1,429K | 2,151K | +13% 증가 |
| Giver v2.1 | 133K | 182K | 975K | 1,424K | -25% 절감 |
| **Giver v2.2** | **79K** | **61K** | **245K** | **423K** | **-78% 절감** |
| 이상적(≤80K) | 80K | 80K | 80K | 320K | -83% 절감 |

> v1, v2가 모놀리식보다 비싼 이유: fork/empty 상속으로 상위 컨텍스트가 누수되어 Scout·Planner 입력이 불필요하게 증가. v2.2에서 `context:"fresh"` 완전 준수로 해결.

**반복 실패 포함 시 절감 증폭:** 동일 버그 4회 시도(Desync Bug) 기준 Giver 1,848K vs 모놀리식(누적) ~13,000K → **86% 절감**. `context:"fresh"`로 인한 실패 시 컨텍스트 격리가 핵심 이점.

### 버전별 달성

**v1 → v2: 격리 복원.** `context:"fresh"` 3% → 100%, fork 8건 → 0건. Worker 버그 수정 8K 🟢 달성. 3-Tier 격리가 설계 대로 작동함을 실증.

**v2 → v2.1: 협업 진단.** Phase 0.5 도입. Planner가 원인 진단이 아닌 구현만 계획하게 변경. Planner 513K → 182K(-64%). 사용자가 "Option B-구조"를 선택한 사례에서 Planner 77K 🟢 — 최초 이상적 달성.

**v2.1 → v2.2: 구조화.** Phase 체크리스트, Scout 3요소 템플릿, 파일 카운트 단계 추가. 규칙 준수를 "판단"에서 "자동 반복"으로 전환. Planner 61K 🟢, Scout 59K 🟢, **모놀리식 대비 -78% 절감** 달성.
