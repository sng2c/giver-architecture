...
### 4.1버전 범위 비교 (체인 아티팩트 측정)

| 메트릭 | v3.6.1 (17체인) | v3.6.2 (4체인) | 변화 |
|--------|:-----------:|:----------:|:----:|
| **P_ctx 평균** | 8,203B | 5,457B | **−34%** |
| **W_ctx 평균** | 4,995B | 2,415B | **−52%** |
| **W_ctx 최대** | 11,527B | 5,707B | **−51%** |
| **Total 평균** | 25,430B | 20,618B | **−19%** |

### 4.2 v3.6.2 체인 상세

| 체인 | P_ctx | W_ctx총 | W_ctx평균 | W_ctx최대 | Total | Workers |
|------|-------|---------|----------|----------|-------|---------|
| 28444dfa | 2,225B | 7,066B | 2,355B | 2,578B | 9,291B | 3 |
| 3e51c1a3 | 7,260B | 21,878B | 2,187B | 4,516B | 29,138B | 10 |
| a2060230 | 7,556B | 19,487B | 3,897B | 5,707B | 27,043B | 5 |
| f670b431 | 4,788B | 12,214B | 1,221B | 4,807B | 17,002B | 10 |

### 4.3 감소 요인 분석

- **W_ctx −52%**: `[Read from:]` 자동 주입으로 Worker 템플릿에서 불필요한 지시문과 대체 로직이 제거됨. no-op Worker도 파일을 찾는 즉시 종료.
- **W_ctx 최대 −51%**: v3.6.1의 이상치(11.5KB)가 해소됨. 대형 과제에서도 5.7KB 이하로 유지.
- **P_ctx −34%**: `[Write to:]` 프리픽스가 "in the chain directory" 같은 모호한 지시를 대체하여 Planner 입력이 간결해짐.
- **Total −19%**: Planner와 Worker 모두 감소하여 전체 효율 개선.

### 4.4 바이트 수치 → 토큰 환산 가이드

바이트 수치는 영문 기준 약 4바이트/토큰으로 환산:
- W_ctx 2,415B ≈ 600토큰 (v3.6.2)
- W_ctx 4,995B ≈ 1,250토큰 (v3.6.1)

### 4.5 주의사항

- v3.6.2 샘플이 4개뿐 (simple task 1개 포함)
- v3.6.1의 과 점수치(11KB+)가 평균을 끌어올림
- 대형 과제(10K+ Task #0)에서 W_ctx은 추가 검증 필요
...
1. **대형 과제 검증**: v3.6.2 실험이 단순 과제(hello world) 1건 포함 4건뿐. 10K+ Task #0에서의 W/P ratio 추가 검증 필요.
2. **v3.6.2 샘플 확대**: 현재 4체인. 10+ 체인에서 통계적 유의성 확보 필요.
...
- **효율**: 최종 컨텍스트 크기 감소 (W_ctx −52%, W_ctx_max −51%)
