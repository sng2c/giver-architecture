...
|---|---|
| 기억 전달자가 모든 기억을 보유 | Tier 1이 모든 대화 컨텍스트를 보유 |
| 수령자는 전달받은 것만 받음 | Tier 2는 컨텍스트 브리프만 수신 |
| 공동체는 Sameness 속에 삶 | Tier 3은 완전히 fresh — 역사 0 |
| 전달은 선택적이고 의도적 | **tx**는 명시적 6섹션 계약 |
| 기억은 사라지지 않고 보류만 됨 | 대화 컨텍스트는 Tier 1에만 머물고 아래로 새지 않음 |
...
| 소스 | 내용 |
|---|---|
| 태스크 스트링 | The Giver가 큐레이션한 6섹션 컨텍스트 브리프 |
...
1. **The Giver가 유일한 컨텍스트 보유자.** 지저분한 대화 기록은 Tier 1에만 있다. 아래로는 절대 흐르지 않는다.

2. **tx가 전달 수단.** Giver는 전체 기억을 쏟지 않는다 — **tx** (transmission)로 6섹션 브리프를 선택적으로 전달한다:
   - **Objective** — 무엇을, 왜
...
3. **실행은 Sameness 속에서.** Planner, scout, worker는 대화 기록 없이 시작한다. 매번 깨끗한 백지. 드리프트도, 노이즈도, 축적된 실수도 없다.

4. **Scout은 항상 worker 앞에.** Fresh worker에는 암묵적 코드 지식이 없다. Scout이 구현 직전에 `context.md`와 `{previous}`로 라이브 코드베이스 길잡이를 제공한다.

5. **Dream Sharing이 실패 반복을 방지한다.** Fresh 에이전트는 이전 실패에 대한 기억이 없다. The Giver가 6섹션 브리프의 `## Previous Failures`를 통해 실패 경험을 전달하지 않으면, 다음 시도는 같은 실수를 반복한다. Dream Sharing은 선택이 아니라 아키텍처의 핵심 품질 메커니즘이다.

6. **주기적 압축이 수렴을 달성한다.** 큐레이션 계층만으로는 선형 성장이 보장되지만, 여전히 ~1K/턴씩 누적된다. 컨텍스트가 무거워지면 Giver가 직접 요약하여 기준선으로 복귀시킨다. 이것이 **톱니 패턴**을 만든다.
...
3. 다음 체인의 컨텍스트 브리프는 압축된 컨텍스트에서 도출
...
**Dream Sharing이 있으면:**
- Worker가 라우트 레이어에 캐시를 구현 → 실패
- Giver가 `## Previous Failures`에 구조화된 실패 기록 전달
...
- **Tier 1 (Giver):** 93턴, P50=21K, 최대 45K, **선형 성장** (10.1×, ~1K/턴)
- **Tier 2/3 (서브에이전트):** 740턴, P50=45K, 최대 117K, fresh 시작
...
>
> planner, worker는 pi-subagents 내장 에이전트의 `.pi/agents/` 오버라이드.

| 파일 | `.pi` 경로 | 링크 | 설명 |
|------|-----------|------|------|
| `giver/SKILL.md` | `.pi/agent/skills/giver/SKILL.md` | [gist](https://gist.github.com/sng2c/9d2c84f8926e63705800498c4e3d2ab9) | The Giver 스킬 — tx 체인, Dream Sharing, 6섹션 계약 |
| `planner.md` | `.pi/agents/planner.md` | [gist](https://gist.github.com/sng2c/04762e1228396dbd996e79a288d13fa4) | pi-subagents 내장 planner 오버라이드 — Fresh Context Mode |
| `worker.md` | `.pi/agents/worker.md` | [gist](https://gist.github.com/sng2c/173f0d4765e15714869d24198cd4b812) | pi-subagents 내장 worker 오버라이드 — Fresh Context Mode |
